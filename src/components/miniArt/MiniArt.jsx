...
export default function MiniArt({postId , post }){
    ...
    const Like = () => {
      axios.post(`https://art-website-server-production.up.railway.app/posts/like/${postId}`, { userId: userId.userId })
          .then((res) => {
            newLike==0?
  ...
  const unLike = () => {
    axios.post(`https://art-website-server-production.up.railway.app/posts/unlike/${postId}`, { userId: userId.userId })
        .then((res) => {
          newLike==0?
          setNewLike(post.likes.counter -1):
          setNewLike(0)
  ...
  const toProfile = () => {
    ...
    useEffect(() => {
      axios.get(`https://art-website-server-production.up.railway.app/users/${post.userId}`)
          .then((result) => {
